...
| 5   | Plataforma                        | **Supabase**: Postgres + autenticación + almacenamiento de ficheros en una sola cuenta     |
...
| Pieza                | Qué aporta                                                        |
| -------------------- | ----------------------------------------------------------------- |
| Supabase Postgres    | Clientes, direcciones, catálogo, noticias y pedidos                |
| Supabase Auth        | Correo y contraseña, recuperación, sesiones. No guardamos claves   |
| Supabase Storage     | Fotos de producto y de noticias                                    |
| Astro + Vercel       | Sigue siendo el sitio. Cambia el modo de renderizado, no el stack  |
| Stripe               | Igual que hoy. El webhook pasa además a escribir el pedido         |
| Resend               | Igual que hoy, para el aviso de pedido al obrador                  |
...
### 5.3 Dónde vive cada cosa después del cambio
...
```
profiles            id (→ auth.users), email, nombre, telefono, rol, created_at
direcciones         id, profile_id, alias, calle, postal_code, predeterminada
...
pedidos             id, profile_id?, stripe_session_id, mode, fecha_entrega, slot?,
...
- **`rol` vive en `profiles`, no en el token.** Se lee en servidor en cada petición que importa.
...
- **`profile_id` es opcional en `pedidos`.** El checkout de invitado sigue existiendo y sus pedidos también se guardan.

## 7. Acceso y permisos

**Autenticación.** Supabase Auth con correo y contraseña. No almacenamos ni ciframos contraseñas: eso es de la plataforma. De serie van recuperación por correo, longitud mínima y límite de intentos contra la pantalla de acceso.

**Autorización en la base de datos.** Las políticas de acceso de Postgres imponen que:

- un `cliente` solo lee y escribe **sus** filas de `profiles`, `direcciones` y `pedidos`;
- un `admin` lee y escribe todo;
- el catálogo y las noticias son **de lectura pública** y solo un `admin` los escribe.

Se impone en la base de datos, no en el navegador. Aunque alguien llame a la API a mano con su sesión, la base de datos le dice que no.
...
- **Copias de seguridad.** Hoy el contenido está en git y se recupera solo. En base de datos hay que activarlas y probarlas.
